...
section_head = """    <section data-bs-version="5.1" class="features9 cid-tvA5SQ6yVu mbr-parallax-background" id="features10-j">
        <!---->
...
def My_Changed_Header():
    return open("file_head.php").read() 
foot = """        
<script>
...
class FileProcessor:
    def __init__(self, bibfile_path, file_data_path, title = "autoencoder"):
        with open(bibfile_path) as bibfile:
            self.bib_db = bibtexparser.load(bibfile)
        
        self.title = title
        self.first_head = self.giveHeader(self.title)
        self.file_data = json.load(open(file_data_path))
        self.colData = ""
    
    
    def giveHeader(self, title):
    ...
    def new_head(self):
        return My_Changed_Header()

    def extract_data(self, rel_path = "", check_path=""):
        for i, entry in enumerate(self.bib_db.entries):
            ...
            try:
                flink = f"{self.file_data[str(i)]}"
                flink = flink if flink in  files else entry.get('url')
            except Exception:
                flink = entry.get('url')
            if flink.startswith('http'):
                colstring = self.make_col_string(title=title, abstract=abstract,author=author, journal=journal, flink=flink, id = i+1)
            else:
                colstring = self.make_col_string(title=title, abstract=abstract,author=author, journal=journal, flink=flink, id = i+1, rel_path=rel_path)
            self.colData += colstring
    
    def make_col_string(self, title, abstract, author, journal, flink, id, rel_path = ""):
        return f"""                <div class="card col-12">
                        <div class="card-wrapper" id="update-div{id}">
                            <div class="top-line row">
                                <div class="col">
                                    <h4 class="card-title mbr-fonts-style display-5"><strong>{title}</strong></h4>
                                    <h7 style="color:blue;"> {journal}</h7>
                                    <p class="text-muted"> <em>{author}</em></p>
                                </div>
                            </div>
                            <hr class="hr5">
                            ...
                            <div class="mbr-section-btn item-footer mt-2 text-center">
                                <a class="slide btn text-center" id="button" target="_blank" href="{rel_path + flink}">&nbsp;
                                </a>
                                <?php if($_SESSION['username'] == "dmcoder") // If session is not set then redirect to Login Page
                        
                {{echo '<a class="slide1 btn text-center"  href="javascript:void(0)" id="button" data-id = "{id}"></a>';}}?>
    ...
    def write_to_file(self, file_path):
        header = self.new_head()
        with open(file_path, "w") as f:
            #f.write(self.first_head)
            f.write(header)
            f.write(section_head)
            f.write(self.colData)
            f.write(section_end)
            f.write(foot)
